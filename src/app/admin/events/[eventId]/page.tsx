  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
        <DashboardPanel
          title="Admin notes"
          description="Capture support and moderation context directly against this event."
        >
          <form action={createAdminNoteAction} className="rounded-3xl bg-canvas p-4">
            <input name="scopeType" type="hidden" value="event" />
            <input name="scopeId" type="hidden" value={detail.event.id} />
            <textarea
              className="min-h-32 w-full rounded-2xl border border-border bg-white px-4 py-3 text-sm text-ink outline-none"
              name="note"
              placeholder="Add an internal note about delivery issues, host support context, or unusual event behavior."
            />
            <div className="mt-3">
        ...
        <DashboardPanel
          title="Recent notes for this event"
          description="Internal notes saved specifically against this event workspace."
        >
          <div className="space-y-3">
            {detail.notes.length ? (
